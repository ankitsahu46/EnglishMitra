import { formatApiTextToHtml } from "@/utils";
import { ContentBlockProps } from "@/types";
import { VolumeBtn, FavExpressionBtn, SynonymAntonymBox } from "@/components";
    ...
    >
      ...
      <div className="flex justify-between items-center w-full">
        <div className="flex flex-col items-start">
          <p className="text-3xl font-semibold text-gray-900/80 gap-1">
            {capitalizeFirstLetter(text)}
            {phonetic && (
              <span className="block text-gray-400/90 italic font-medium text-sm">
                {phonetic}
              </span>
            )}
          </p>
      ...
      <div className="mt-1 pt-2">
          ...
          <p
            ...
            dangerouslySetInnerHTML={{
              __html: capitalizeFirstLetter(
                formatApiTextToHtml(definition ?? "")
              ),
            }}
          />
        )}
        ...
        {example && (
          <p
            ...
            dangerouslySetInnerHTML={{
              __html: capitalizeFirstLetter(formatApiTextToHtml(example ?? "")),
            }}
          />
        )}
      </div>
      ...
      <div className="flex flex-col self gap-2 mt-10">
        {Array.isArray(synonyms) && synonyms?.length > 0 && (
          <SynonymAntonymBox type={"synonyms"} content={synonyms} />
        )}
        {Array.isArray(antonyms) && antonyms?.length > 0 && (
          <SynonymAntonymBox type={"antonyms"} content={antonyms} />
        )}
      </div>
    </div>
  );
